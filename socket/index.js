...
const initSocket = (server, corsOptions) => {
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("SEND_MESSAGE", async (messageData) => {
      ...
      if (clientId) {
        if (!messageData.imageUrl) {
          ...
          // 将图片数据和消息内容发送给所有在线用户
          socket.to(clientId).emit("RECEIVE_MESSAGE", {
            ...messageData,
            imageData: {
              data: imageBuffer,
            },
    ...
    io.on("UPDATE_MESSAGE_STATUS", ({ type, readerId, messageSender }) => {
      // messageSender 的訊息被 readerId 已讀
      const socketId =
        type === "room"
          ? messageSender
          : onlineUsers.find(({ userId }) => userId === messageSender)
              ?.socketId;
      if (socketId) {
        socket
          .to(socketId)
      ...
      invitedUser.forEach((invitedUser) => {
        ...
        if (socketId) {
          socket.to(socketId).emit("INVITED_TO_ROOM", {
            message: `${creator} 已將你加入 ${name} 聊天室`,
          });
        }
      });
    });
